	...
	def post(self, request):
		...
		if serializer.is_valid(raise_exception=True):
			...
			try:
				...
				return Response({'nombre_usr': request.user.nombres, 'cartera': False}, status=status.HTTP_200_OK)
			else:
				return Response({'nombre_usr': request.user.nombres, 'cartera': True}, status=status.HTTP_200_OK)
